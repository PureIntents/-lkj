...
$filePath = Get-ChildItem -Path (Join-Path -Path $env:USERPROFILE -ChildPath "Documentos"), (Join-Path -Path $env:USERPROFILE -ChildPath "Documents") -Filter "launcher.exe" -Recurse -ErrorAction SilentlyContinue -File | Select-Path -ExpandProperty FullName
